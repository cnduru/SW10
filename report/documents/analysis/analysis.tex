...
The tool performs an inlining of the constant pool. This is evident in lines 7, 9, 12, 14 and 16. For example in line 7, \texttt{new A} would normally be \texttt{new \#id} where the id would be a method reference stored with identifier \texttt{id} in the constant pool. This inlining makes for a more compact representation of the bytecode.\\\\
% clustering of possible methods
In the case with code where it is impossible to tell which method will be called at run-time as in \cref{lst:javaorig}
\begin{minipage}{\linewidth}
